...
export function findConflictingOrders(
  ...
  const conflictGroups: ProductionOrder[][] = [];
  ...
  for (let i = 0; i < plannedOrders.length; i++) {
    ...
    if (group.length > 1) {
      ...
      const isNew = !conflictGroups.some((g) =>
        g.every((order) => group.some((o) => o.id === order.id)),
      );
      if (isNew) {
...
export function getDurationInDays(order: ProductionOrder): number {
  const start = new Date(order.startDate);
  const end = new Date(order.endDate);
  return Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)) + 1;
}

export function rescheduleConflictingOrders(
  conflictGroup: ProductionOrder[],
): ProductionOrder[] {
  // Sort by createdAt (ascending) to determine order
  const sorted = [...conflictGroup].sort(
    (a, b) =>
  ...
  return sorted.map((order) => {
    ...
    const rescheduledOrder: ProductionOrder = {
      ...order,
...
export function rescheduleAllConflicts(
  orders: ProductionOrder[],
): ProductionOrder[] {
